...
#         from utils.datasets_single import create_dataloader
        from utils.datasets import create_dataloader_sr_AITOD as create_dataloader
    ...
    # else:
        ...
        #                                 image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    mlc = np.concatenate(dataset.labels, 0)[:, 4].max()  # max label class
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,default='models/SRyolo_noFocus_small.yaml', help='model.yaml path') #yolov5s
    parser.add_argument('--super', action='store_true', help='super resolution')
    parser.add_argument('--data', type=str,default='data/SRAITOD.yaml', help='data.yaml path')
    ...
    parser.add_argument('--train_img_size', type=int,default=800, help='train image sizes,if use SR,please set 1024') #512
    parser.add_argument('--test_img_size', type=int, default=800, help='test image sizes') #512
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        opt.data, opt.cfg, opt.hyp = check_file(opt.data), check_file(opt.cfg), check_file(opt.hyp)  # check files  [ATTN] data checkfile
    ...
    if not opt.evolve:
        ...
        train(hyp, opt, device, tb_writer)  # [ATTN] train
