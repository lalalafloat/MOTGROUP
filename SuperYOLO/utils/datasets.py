...
from SuperYOLO.utils.general import xyxy2xywh, xywh2xyxy, clean_str
from SuperYOLO.utils.torch_utils import torch_distributed_zero_first
...
def create_dataloader_sr_AITOD(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix=''):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels_sr_AITOD(path, imgsz, batch_size,
                                      augment=augment,  # augment images
                                      hyp=hyp,  # augmentation hyperparameters
                                      rect=rect,  # rectangular training
                                      cache_images=cache,
...
def img2label_paths(img_paths):
    ...
    return [x.replace('images', 'labels', 1).replace('.png', '.txt') for x in img_paths] #replace('.' + x.split('.')[-1], '.txt')
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with open(path, "r") as file:
            ...
            for j in range(len(self.img_files)):
                self.img_files[j] = self.img_path + self.img_files[j].rstrip() + '.png' #self.img_files[j].rstrip() + '_co.png'  #self.img_path + self.img_files[j].rstrip() + '_co.png'
...
class LoadImagesAndLabels_sr_AITOD(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        self.img_size = img_size
        self.augment = augment
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        ...
                        #     class2index = {'airplane':0, 'bridge':1, 'storage-tank':2,'ship':3, 'swimming-pool':4, 'vehicle':5, 'person':6, 'wind-mill':7} # class2index
                        #     l[i] = [class2index[j] if j in class2index else j for j in l[i]] # replace class name with class index
                        #     l[i][0],l[i][1],l[i][2],l[i][3],l[i][4] = l[i][4],l[i][0],l[i][1],l[i][2],l[i][3]
                        #     for j in range(1,5):
                        #         l[i][j] = float(l[i][j])/800
                        #     w = l[i][3] - l[i][1]
                        #     h = l[i][4] - l[i][2]
                        #     l[i][1] = l[i][1] + w/2
                        #     l[i][2] = l[i][2] + h/2
