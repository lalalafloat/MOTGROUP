...
from utils.datasets import create_dataloader, create_dataloader_sr, create_dataloader_sr_AITOD
...
#     print(model)
    ...
    if not training:
        ...
        dataloader = create_dataloader_sr_AITOD(data[task], imgsz, batch_size, gs, opt, pad=0.5, rect=True,
    ...
    for batch_i, (img, ir, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)): #zjq #targets : [[img_id, cls, box(4)]]
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            out = non_max_suppression(out, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True) #(box(0,1,2,3),conf,cls)
            # out = weighted_boxes(out,image_size=imgsz, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True)
            t1 += time_synchronized() - t

        # Statistics per image
        for si, pred in enumerate(out): #si : img_id 
    ...
    if save_json and len(jdict):
        ...
        anno_json = '/media/ok/4t1/Data/AI-TOD/annotations/aitod_test_v1.json'  # annotations json
        ...
        with open(anno_json, "r") as f:
            anns = json.load(f)
        imagenames2id = {ann['file_name'][:-4]: ann["id"] for ann in anns["images"]}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/SRAITOD.yaml', help='*.data path')
